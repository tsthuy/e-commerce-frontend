...
import type { StripeAuBankAccountElementChangeEvent } from '@stripe/stripe-js';
...
type CustomInputCardBankAccountProps = Partial<InputHTMLAttributes<HTMLInputElement>> & {
  name: string;
  ...
  handleOnChange?: (value: StripeAuBankAccountElementChangeEvent) => void;
};

export const CustomInputCardBankAccount = memo((props: CustomInputCardBankAccountProps) => {
  const {
    name,
    className,
    classNameInput,
      ...
      render={({ field }) => (
        <FormItem className={cn('custom-form-item__wrapper', formItemClassName, { group: labelStyle === 'dynamic' }, className)}>
          ...
          <CustomFormInputWrapper readOnly={readOnly}>
            <CustomFormControl className={cn('custom-form-control__card-bank-account', classNameInput)} readOnly={readOnly}>
