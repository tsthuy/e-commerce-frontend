...
export class ConversationService {
  ...
  static async checkConversationExists(customerId: string, sellerId: string): Promise<boolean> {
    try {
      const conversationId = `${customerId}_${sellerId}`;
      const docRef = doc(firestore, `allConversation/${customerId}/conversations`, conversationId);
  ...
  static async createConversation(params: CreateConversationParams): Promise<string> {
    try {
      ...
      const customerConversationRef = doc(firestore, `allConversation/${customerId}/conversations`, conversationId);
      ...
      const sellerConversationRef = doc(firestore, `allConversation/${sellerId}/conversations`, conversationId);
  ...
  static async sendMessage(params: SendMessageParams): Promise<string> {
    try {
      ...
      const senderConversationRef = doc(firestore, `allConversation/${senderId}/conversations`, conversationId);
      ...
      const receiverConversationRef = doc(firestore, `allConversation/${receiverId}/conversations`, conversationId);
  ...
  private static async ensureConversationExists(conversationId: string, customerId: string, sellerId: string, initiatorType: 'customer' | 'seller'): Promise<void> {
    try {
      ...
      // Check if customer's conversation exists
      const customerConversationRef = doc(firestore, `allConversation/${customerId}/conversations`, conversationId);
      const customerDoc = await getDoc(customerConversationRef);

      if (!customerDoc.exists()) {
        // Create conversation metadata for customer
        await setDoc(customerConversationRef, {
          conversationId,
          isSeen: initiatorType === 'customer', // Customer sees it if they initiated
          lastMessage: '',
          receiverId: sellerId,
      ...
      // Check if seller's conversation exists
      const sellerConversationRef = doc(firestore, `allConversation/${sellerId}/conversations`, conversationId);
      const sellerDoc = await getDoc(sellerConversationRef);

      if (!sellerDoc.exists()) {
        // Create conversation metadata for seller
        await setDoc(sellerConversationRef, {
          conversationId,
          isSeen: initiatorType === 'seller', // Seller sees it if they initiated
          lastMessage: '',
          receiverId: customerId,
  ...
  static subscribeToConversations(params: ConversationListParams, callback: (conversations: ConversationMetadata[]) => void): () => void {
    try {
      const { userId, limit: pageLimit = 20 } = params;

      const conversationsRef = collection(firestore, `allConversation/${userId}/conversations`);
  ...
  static async markConversationAsSeen(userId: string, conversationId: string): Promise<void> {
    try {
      const conversationRef = doc(firestore, `allConversation/${userId}/conversations`, conversationId);
  ...
  static async getConversationMetadata(userId: string, conversationId: string): Promise<ConversationMetadata | null> {
    try {
      const conversationRef = doc(firestore, `allConversation/${userId}/conversations`, conversationId);
